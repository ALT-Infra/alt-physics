...
symmetric relationships, ports, prior positions, and hard pins; it returns node
centers, obstacle-safe routes, metrics, and solver diagnostics.
...
3. Minimize stress, hierarchy, repulsion, overlap, and temporal-prior energies
   with maintained L-BFGS and line-search implementations from `argmin`.
4. Project rectangular non-overlap exactly, then polish without moving fixed
   nodes.
